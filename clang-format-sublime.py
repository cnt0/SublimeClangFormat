...
# sublime.save_settings("ClangFormat.sublime-settings")
print(sublime.load_settings("ClangFormat.sublime-settings").get("clangformatpath"))
...
class ClangEV(sublime_plugin.EventListener):
...
class ClangFormatCommand(sublime_plugin.TextCommand):
  ...
  def is_enabled(self):
    return not re.match(".*\.(c|cc|cpp|h|hpp)$", self.view.file_name()) is None
  ...
  def run(self, edit):
    ...
    if encoding == 'Undefined':
      encoding = 'utf-8'
    regions = []
    command = [binary, '-style', style, '-assume-filename', str(self.view.file_name())]
